...
public class UpdateItemCommandHandlerTests : IClassFixture<StoreApiFixture>
{
	...
	{
		// Arrange
		var expectedItem = new ItemDto(SeedModels.Item1) with
		{
			QuantityInStock = 15,
			Price = 9.99m,
	...
	[Fact]
	public async Task UpdateItemAsync_ShouldNotChangeItem_WhenUpdateDtoHasNullParams()
	{
		// Arrange
		var expectedItem = new ItemDto(SeedModels.Item1);
		var dto = new UpdateItemDto(SeedModels.Item1.Id, null, null, null, null);
		var command = new UpdateItemCommand(dto);
